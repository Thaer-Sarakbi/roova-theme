...
Statuses `hold | pending | confirmed | cancelled`. **Only `hold` and `confirmed` occupy inventory**
(`Roova_Availability::active_statuses()`, filterable as `roova_active_booking_statuses`): a room is
taken while it sits in a cart, and taken again once the order behind it is *paid*. `pending` — an
order placed and not paid for — deliberately holds nothing. Letting it hold dates meant anyone could
empty a hotel by walking up to the Place order button and stopping there, and it meant a guest read
"booked" for a stay that was one abandoned payment from being released. Payment is what books the
room, and the mapping in `Roova_Orders::map_status()` is the only place that decides it.

The cost of that is real and accepted: between an order being placed and paid, its rooms are back on
sale, so two guests can pay for the last one. That is why `flag_overbooking()` exists — see *Race
protection*. Bank transfer (`on-hold`) is no exception; money not yet received is a room not yet sold.

Availability queries exclude anything past `expires_at` **in SQL**, so correctness never depends on the
cleanup cron firing on time — the cron is tidy-up only. Since a `pending` row holds nothing, its expiry
(`roova_pending_order_minutes`, the Customizer's "Unpaid order expiry") is *only* tidy-up: it is when
the row stops being listed as live on the Bookings screen.
...
cart/checkout view → locked check at `woocommerce_checkout_order_processed` (and the Store API
equivalent, which rethrows as `RouteException`) → order status mapping. A conflict at that check fails
the order and throws so checkout errors out.

That third layer **checks the stay; it does not book it**. `commit_order()` attaches the holds to the
order and leaves them `pending`, which holds no dates — see *Availability*. The room is only taken when
the order is paid, so the last layer, the status mapping, is the one that books it. A *paid* order that
...
own Contact card in the hotel page sidebar, under the map. The field had been stored and editable for
a long time and shown nowhere — the card is what made it mean anything.

- **No number, no card.** An empty field leaves the section off rather than printing an empty panel,
  which is why the admin description says so: a client should not have to guess what clearing it does.
- **`roova_tel_href()` keeps the digits and a leading `+` and nothing else.** A client writes the
  number to be *read* — spaces, brackets, dashes — and every one of those either does nothing in a
  dialler or stops the `tel:` link working. The number is still displayed exactly as typed; only the
  href is reduced. It returns an empty string when there are no digits at all, so a field holding
  something like "ask at the desk" is printed as plain text instead of becoming a dead link.
- **The number is gold, and that is what fixes its size and weight.** Gold on the white card measures
  3.39:1 — under AA for normal text, over the 3:1 that large text is held to. So it is set the way the
  booking box a card above sets its price: `--roova-font-body` at 1.45rem/700, which is past the
...
A tier carries two kinds of benefit. The **written** ones stay display-only — promises the hotel keeps
at the desk, which is why each is text the client types rather than logic the theme runs. Two are the
exception and really do come off a total: **free nights** (`free_nights`, whole nights, capped at 365)
and a **checkout discount** (`discount`, 0–100, two decimals). Both are applied by
`roova_vip_apply_cart_discount()` on `woocommerce_cart_calculate_fees`, each as its own line in the
order summary.

- **Negative fees, not coupons.** A coupon needs a code, appears in the coupon row with a "remove"
  link beside it, and would have to exist in the database for every tier. A fee is recalculated on
  every cart recalculation, so it follows the rooms, the quantity and any coupon already applied.
- **They are taxable, in the rooms' own tax class**, read off the first cart line. That is what makes
  the tax rows fall with them — a stay discounted by 10% is taxed on what is actually charged — and
  what makes the grand total drop by exactly the discount. Verified live: $630 subtotal → −$63.00,
  Tourism Tax $28.35, SST $56.70, total $652.05, which is 630 × 1.15 × 0.9.
- **The base is the sum of `line_total`, not `get_subtotal()`.** `line_total` is what a room costs
  *after* a coupon and is settled before fees are calculated; taking the percentage off the pre-coupon
  subtotal would let a coupon and a tier together discount more than the stay is worth.
- **Both are measured against that same base — they are not stacked.** WooCommerce sorts fee lines by
  amount (`WC_Cart_Fees::sort_fees_callback()` is `$a->amount > $b->amount ? -1 : 1`), so which of the
  two prints first is not ours to choose, and a line that only adds up underneath the other one would
  read as an arithmetic error half the time. Ten percent off is ten percent of the subtotal above it,
  whichever line it lands under. The pair is still held to the price of the stay: the free nights are
  taken at full value and the percentage takes whatever is left, so an extravagant tier lands on a
  total of zero rather than below one.
- **A free night is one room for one night** at the quoted nightly rate, capped at the length of the
  booking — `roova_vip_free_nights_credit()`. It is *not* multiplied by the number of rooms: "two
  nights free" is two nights off the stay, not two off every room in it. Where several booking lines
  exist (only possible with `roova_single_item_cart` filtered off) the dearest room is the one
  credited, because that is the night a guest would choose.
- **The count is trimmed with the amount, never after it.** If a cap bites, `nights` comes down with
  the money, so a guest is never told two nights are free while one came off.
- **The figures live inside the fees' names** (`roova_vip_discount_label()` → "VIP Gold discount
  (10%)", `roova_vip_free_nights_label()` → "VIP Gold: 2 free nights"), because those lines are read in
  a dozen places the theme does not draw — WooCommerce's order screen, the customer email, a refund —
  and the name is the only part that travels. `checkout.php` and `inc/order.php` both add their
  discount class to any fee whose total is negative, so both read as discounts rather than charges.

`roova_vip_discount_percent()` and `roova_vip_free_nights()` are the one door to each figure; both
return nothing for a signed-out visitor, and `roova_vip_discount_enabled` switches the money off while
leaving the tiers and the rail alone. The VIP tab draws both as its **first two benefits**, generated
...
Cashback is **display only** — a number the theme keeps and shows, the way a VIP tier's *written*
benefits are shown. It is not the VIP checkout discount, which really does come off a total.
...
**The rooms in the summary have no delete button**, and there is no remove/undo AJAX pair behind one
any more (`roova_ajax_remove_cart_item()` / `roova_ajax_restore_cart_item()`, the `.roova-summary__remove`
button, its undo slot and their CSS and JS are all gone). Asked for directly — don't put it back. The
cart carries a single booking (*One booking per cart*), so the only thing that button could do was
empty the checkout and bounce the guest to the cart page, and it sat one mis-tap away from the line
total. Changing the stay means going back to the hotel page, which re-books through the one door every
